...
const RentalPaymentsMonitoring = () => {
  ...
  useEffect(() => {
    ...
    const fetchPayments = async () => {
      ...
      const tenantIds = new Set(data.map((payment: Payment) => payment.tenantId));
      for (const id of tenantIds) {
        const tenantResponse = await fetch(`/api/tenants/${id}`);
        const tenantData = await tenantResponse.json();
        setTenants((prev) => ({ ...prev, [id]: tenantData.name }));
      }
    };
  ...
  const handleAddPayment = async (e: React.FormEvent) => {
    ...
    if (response.ok) {
      ...
      setPayments([...payments, { id: data.id, tenantId: Number(tenantId), amount: Number(amount), date, settled }]);
      ...
      <table className="min-w-full bg-white border border-gray-200">
        ...
        <tbody>
          {payments.map((payment) => (
            <tr key={payment.id}>
              <td className="px-4 py-2 border-b text-black">{payment.tenantId}</td>
              <td className="px-4 py-2 border-b text-black">{tenants[payment.tenantId] || 'Loading...'}</td>
