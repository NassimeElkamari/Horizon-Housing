...
const TenantManagement = () => {
  ...
  const [isEditing, setIsEditing] = useState(false); // Manage form mode (add/edit)
  const [editTenantId, setEditTenantId] = useState<number | null>(null); // Tenant being edited
  ...
  const handleUpdateTenant = async (e: React.FormEvent) => {
    ...
    if (editTenantId === null || selectedProperty === null) {
      setError('No tenant selected for updating.');
      return;
    }

    const response = await fetch(`/api/tenants/${editTenantId}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ name, contact_details: contactDetails, section }),
    });

    if (response.ok) {
      setTenants(tenants.map((tenant) =>
        tenant.id === editTenantId
          ? { ...tenant, name, contact_details: contactDetails, section }
          : tenant
      ));
      resetForm();
    ...
    <div className="p-6">
      ...
      {selectedProperty !== null && (
        <div>
          ...
          >
            {isFormVisible ? 'Hide Form' : 'Add Tenant'}
          </button>

          {isFormVisible && (
            <form onSubmit={isEditing ? handleUpdateTenant : handleAddTenant} className="space-y-4 mb-6">
              <div>
